...
module.exports.onLoad = () => {
  ...
  if (!fs.existsSync(__dirname + "/data/shortvideo.json")) fs.writeFileSync(__dirname + "/data/shortvideo.json", JSON.stringify([]), 'utf-8');
}
...
module.exports.handleEvent = async function({ api, event }) {
  ...
  if (event.type !== "message_unsend" && event.body.length !== -1) {
    let shortcut = JSON.parse(fs.readFileSync(__dirname + "/data/shortvideo.json"));
      ...
      if (getThread.some(item => item.in == event.body)) {
        ...
        if (typeof shortOut == "string") {
          ...
          if (shortOut.indexOf(" | ") !== -1) {
            ...
            let shortvideo = (await axios.get( `${shortlink[i]}`, { responseType: "arraybuffer" })).data;
            fs.writeFileSync( __dirname + `/data/shortvideo${i}.mp4`, Buffer.from(shortvideo, "utf-8"));
            ...
            path.push(fs.createReadStream( __dirname + `/data/shortvideo${i}.mp4`));
            }
            var mainpath = [...path];
            api.sendMessage({attachment: mainpath}, event.threadID);  
            for (let i = 0; i < mainpath.length; i++) {
              fs.unlinkSync(__dirname + `/data/shortvideo${i}.mp4`)
            ...
            let shortvideo = (await axios.get( `${shortOut}`, { responseType: "arraybuffer" })).data;
            fs.writeFileSync( __dirname + `/data/shortvideo.mp4`, Buffer.from(shortvideo, "utf-8"));
            api.sendMessage({attachment: fs.createReadStream( __dirname + `/data/shortvideo.mp4`)}, event.threadID, () => fs.unlinkSync(__dirname + `/datashortvideo.mp4`));             }
        }
        else if (typeof shortOut == "object") {
          var path = [];
          for (let i = 0; i < shortOut.length; i++) {
          let shortvideo = (await axios.get( `${shortOut[i]}`, { responseType: "arraybuffer" })).data;
          fs.writeFileSync( __dirname + `/data/shortvideo${i}.mp4`, Buffer.from(shortvideo, "utf-8"));
          path.push(fs.createReadStream( __dirname + `/data/shortvideo${i}.mp4`));
          }
          var mainpath = [...path];
...
module.exports.run = async function({ api, event, args }) {
  ...
  if (content.indexOf(`xóa`) == 0 || content.indexOf(`xoá`) == 0|| content.indexOf(`del`) == 0) {
    ...
    return fs.readFile(__dirname + "/data/shortvideo.json", "utf-8", (err, data) => {
      if (err) throw err;
      ...
      fs.writeFile(__dirname + "/data/shortvideo.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("Đã xóa shortvideo thành công!", threadID, messageID));
    });
  }
    ...
    return fs.readFile(__dirname + "/data/shortvideo.json", "utf-8", (err, data) => {
      ...
      if (allData.some(item => item.id == threadID)) {
        ...
        return fs.writeFile(__dirname + "/data/shortvideo.json", JSON.stringify(getThread), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("Đã xóa toàn bộ shortvideo" , threadID, messageID));
      }
    }); 
  else {
    ...
    if (!shortout) {
      if(event.type == "message_reply" &&event.messageReply.attachments[0].type == "video" ) {
        ...
        for (let i = 0; i < att.length; i++) {
          let link = await short((event.messageReply.attachments[i] != "") ? (event.messageReply.attachments[i].type == "video") ? event.messageReply.attachments[i].url : "":"");
          shortout.push (link)
        }
      }else return api.sendMessage("Thiếu output", threadID, messageID); 
    }//return api.sendMessage("Thiếu output", threadID, messageID);
    return fs.readFile(__dirname + "/data/shortvideo.json", "utf-8", (err, data) => {
      if (err) throw err;
      var oldData = JSON.parse(data);
      if (!oldData.some(item => item.id == threadID)) {
        ...
        return fs.writeFile(__dirname + "/data/shortvideo.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("Tạo shortvideo thành công", threadID, messageID));
      }
      else {
