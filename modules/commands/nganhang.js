...
module.exports.run = async function ({ api, event, args, Currencies, Users }) {
    ...
    const checkBank = (await axios.get(`https://api-dien-2.kimle6.repl.co/bank/check?ID=${senderID}`)).data   
    const { createReadStream } = require(`fs-extra`);
    switch(args[0]) {
        case 'register':
        case '-r':
        case 'r': {
            const res = (await axios.get(`https://api-dien-2.kimle6.repl.co/bank/register?senderID=${senderID}&name=${encodeURI((await Users.getData(senderID)).name)}`)).data
        ...
        case "-f": {
            ...
            let { data } = (await axios.get(`https://api-dien-2.kimle6.repl.co/bank/find?type=${args[1].toUpperCase()}&${args[1].toUpperCase()}=${args[2]}`))
      ...
      case '-c': {
        ...
        const res = (await axios.get(`https://api-dien-2.kimle6.repl.co/bank/find?type=ID&ID=${a}`)).data  
        ...
        case 'rút': {
         ...
         case '-t':{
            ...
            const res = (await axios.get(`https://api-dien-2.kimle6.repl.co/bank/top`)).data  
        ...
        case 'nạp': {
            ...
            const res = (await axios.get(`https://api-dien-2.kimle6.repl.co/bank/send?senderID=${senderID}&money=${args[1]}`)).data  
            return api.sendMessage(`${res.message.noti}\n👤 Chủ tài khoản: ${res.message.name}\n💰 Số dư hiện tại: ${res.message.money}$`, threadID, messageID)
            break;
    }
        case 'password':
        case 'pw': {
            if(checkBank.status == false) return api.sendMessage('Bạn chưa có tài khoản AGRIBANK!', threadID, messageID);
            var type = args[1];
            switch(type) {
                case 'get': {
                    const res = (await axios.get(`https://api-dien-2.kimle6.repl.co/bank/password?bka=${type}&dka=${senderID}`)).data 
                    api.sendMessage('Mật khẩu của b���n được gửi đến tin nhắn chờ', threadID, messageID);
...
module.exports.handleReply = async function ({ api, event, handleReply, Currencies }) {
    ...
    switch(handleReply.type) {
        case 'paySTK': {
            const res = (await axios.get(`https://api-dien-2.kimle6.repl.co/bank/pay?type=STK&senderID=${senderID}&STK=${handleReply.STK}&money=${handleReply.money}&password=${body}`)).data 
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
            api.sendMessage(`${res.message.noti}\n${res.message.data.message}`, threadID, messageID);
            return api.sendMessage(`${res.message.noti}\n\n${res.message.data.message}`, handleReply.threadID);
        }
        case 'payID': {
            const res = (await axios.get(`https://api-dien-2.kimle6.repl.co/bank/pay?type=ID&senderID=${senderID}&userID=${handleReply.ID}&money=${handleReply.money}&password=${body}`)).data 
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
            api.sendMessage(`${res.message.noti} ${res.message.data.message}`, threadID, messageID);
            return api.sendMessage(`${res.message.noti}\n\n${res.message.data.message}`, handleReply.threadID);
        }
        case 'getMoney': {
            const res = (await axios.get(`https://api-dien-2.kimle6.repl.co/bank/get?ID=${senderID}&money=${handleReply.money}&password=${body}`)).data  
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
        ...
        case 'newPassword': {
            const res = (await axios.get(`https://api-dien-2.kimle6.repl.co/bank/password?bka=recovery&dka=${senderID}&fka=${body}`)).data  
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
